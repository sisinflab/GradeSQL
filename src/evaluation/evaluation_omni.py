...
"""Taken and adapted from official OmniSQL repo."""
...
def mark_invalid_sqls(db_files, sqls):
    ...
    execution_results = []
    execute_sqls_parallel(db_files, sqls, num_cpus=20, timeout=30)
...
def major_voting(db_files, pred_sqls, sampling_num, return_random_one_when_all_errors=True):
    ...
    execute_sqls_parallel(db_files, pred_sqls, num_cpus=20, timeout=30)
    execution_results = sorted(execution_results, key=lambda x:x['data_idx'])
...
def run_eval(gold_file, pred_file, db_path, mode, save_pred_sqls, num_cpus=20, timeout=30):
    ...
    if mode == "greedy_search":
        ...
        print("Number of generations (N):", sampling_num)
