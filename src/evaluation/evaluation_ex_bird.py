...
"""Taken and adapted from official BIRD repo."""


def load_json(dir, num_queries='ALL'):
    with open(dir, 'r') as j:
        contents = json.loads(j.read())
    
    if num_queries != 'ALL':
        return contents[:num_queries]
    else:
        return contents
...
def execute_model(predicted_sql, ground_truth, db_place, idx, meta_time_out):
    try:
        ...
        return {'sql_idx': idx, 'res': res, 'status': 'success'}
    except KeyboardInterrupt:
        sys.exit(0)
    except FunctionTimedOut:
        return {'sql_idx': idx, 'res': 0, 'status': 'timeout'}
...
def result_callback(result):
    ...
    if result['status'] == 'timeout':
        print(f"Timeout in SQL {result['sql_idx']}:")



def package_sqls(sql_path, db_root_path, mode='gpt', data_mode='dev', num_queries='ALL'):
    clean_sqls = []
    db_path_list = []
    if mode == 'gpt':
        sql_data = json.load(open(sql_path + 'predict_' + data_mode + '.json', 'r'))
        if num_queries != 'ALL':
            sql_data = dict(list(sql_data.items())[:num_queries])
...
def compute_acc_by_diff(exec_results,diff_json_path, num_query):
    num_queries = len(exec_results)
    results = [res['res'] for res in exec_results]
    contents = load_json(diff_json_path, num_query)
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--num_queries',type=str,default='ALL')
    ...
    simple_acc, moderate_acc, challenging_acc, acc, count_lists = \
        compute_acc_by_diff(exec_result,args.diff_json_path, num_queries)
